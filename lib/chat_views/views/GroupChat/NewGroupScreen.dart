...
//    List<String> selectedNewMember = getStringListAsync(selectedMember)!;
    ...
    await _firestore.collection(GROUP_COLLECTION).doc(widget.groupId).update({
      ...
      chatMessageService.getContactsDocument(of: userID, forContact: widget.groupId).set(data.toJson()).then((value) {}).catchError((e) {
        log(e);
      });
    ...
    return Scaffold(
      appBar: appBarWidget(
        ...
        titleWidget: widget.isAddParticipant
            ? Text('lblAddparticipants', style: boldTextStyle(color: Colors.white, size: 18, letterSpacing: 0.5))
            : Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                mainAxisAlignment: MainAxisAlignment.end,
                children: [Text('lblCreateGroup', style: boldTextStyle(color: Colors.white, size: 18, letterSpacing: 0.5)), 4.height, Text('lblAddparticipants', style: secondaryTextStyle(color: Colors.white, letterSpacing: 0.5))],
              ),
        actions: [
          AnimatedContainer(
      ...
      body: StreamBuilder<List<UserModel>>(
        ...
        builder: (_, snap) {
          if (snap.hasData) {
            ...
            return Container()/*UserListComponent(
              snap: snap,
              isGroupCreate: true,
