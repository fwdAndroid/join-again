...
class GroupChatMessageService extends BaseService {
  ...
  addGroup(GroupModel data, DateTime time) async {
    await fireStore.collection('groups').doc(userID.toString() + '--' + time.millisecondsSinceEpoch.toString()).set(data.toJson());
  }
  ...
  Future<void> addMessageToDb(
    ...
    if (image != null) {
      ...
      Reference storageRef = _storage.ref().child("$GROUP_PROFILE_IMAGES/userId/$fileName");
    ...
    userRef.doc(userID).update({"lastMessageTime": DateTime.now().millisecondsSinceEpoch});
  ...
  addLatLong(ChatMessageModel data, {String? lat, String? long, String? groupId}) {
    ...
    fireStore
        ...
        .doc(data.id)
        .set({'currentLat': lat, 'currentLong': long}, SetOptions(merge: true)).then((value) {
      //
    });
  ...
  Future<void> clearAllMessages({String? groupDocId}) async {
    ...
    fireStore.collection(GROUP_COLLECTION).doc(groupDocId).collection(GROUP_CHATS).get().then((value) async {
