...
class ChatMessageService extends BaseService {
  ...
  Future<DocumentReference> addMessage(ChatMessageModel data,bool isReverse) async {
    print('data:${data.senderId}:${data.receiverId}');
    var doc = await ref!.doc(isReverse?data.receiverId:data.senderId).collection(isReverse?data.senderId!:data.receiverId!).add(data.toJson());
  ...
  Future<void> addMessageToDb({required DocumentReference senderDoc, required ChatMessageModel data, UserModel? sender, UserModel? user, File? image, bool isRequest = false}) async {
    ...
    if (image != null) {
      ...
      Reference storageRef = _storage.ref().child("$CHAT_DATA_IMAGES/userId/$fileName");
  ...
  Stream<int> getUnReadCount({String? senderId, required String receiverId, String? documentId}) {
    print('receiverId:$receiverId:senderId:$senderId');
    if (!(senderId == userID)) {
      return ref!
          .doc(senderId)
