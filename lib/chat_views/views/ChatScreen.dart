  ...
  void sendNormalMessages(ChatMessageModel data, {FilePickerResult? result, File? filepath}) async {
    ...
    try {
      ...
      SendNotification().sendNoti(
          title: userData.name.toString() + " sent you ${messageCont.text.isEmpty ? 'an image' : 'a message'}",
          body: messageCont.text,
          fcmToken: widget.receiverUser!.fcmToken.toString());
    } catch (e) {}
                ...
                children: [
                  iconsBackgroundWidget(context, name: "Gallery", iconData: Icons.image, color: Colors.purple.shade400).onTap(() async {
                    ...
                    FilePickerResult? result =
                        await FilePicker.platform.pickFiles(type: FileType.image, allowMultiple: true, allowCompression: true);
                    ...
                    if (result != null) {
                      ...
                      // finish(context);
                      bool res = await MultipleSelectedAttachment(attachedFiles: image, userModel: widget.receiverUser, isImage: true)
                          .launch(context);
                      if (res) {
                        finish(context);
                        print("Step1");
//                        await ChatScreen(widget.receiverUser).launch(context);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar:
          PreferredSize(preferredSize: Size(context.width(), kToolbarHeight), child: ChatAppBarWidget(receiverUser: widget.receiverUser!)),
      body: Container(
            ...
            children: [
              Container(
                decoration: boxDecorationWithShadow(
                    borderRadius: BorderRadius.circular(30), spreadRadius: 0, blurRadius: 0, backgroundColor: context.cardColor),
...
//                         _showAttachmentDialog();
              ...
              GestureDetector(
                onTap: () {
                  if (messageCont.text.trim().isNotEmpty) sendMessage();
                },
                child: Container(
                  width: 50,
                  height: 50,
                  ...
                  decoration: BoxDecoration(
                      color: messageCont.text.trim().isNotEmpty ? Theme.of(context).primaryColor : Colors.grey.withOpacity(0.4),
